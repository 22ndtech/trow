...
pub const SUPPORTED_DIGEST_ALGORITHMS: [DigestAlgorithm; 2] = [DigestAlgorithm::Sha256, DigestAlgorithm::Sha512];
...
pub fn hash_tag<R: Read>(algo: &DigestAlgorithm, reader: R) -> Result<String, APIRegistryError> {
    ...
    match algo {
        DigestAlgorithm::Sha256 => {
            sha256_tag_digest(reader)
                .map_err(|_e| {
                    let message = format!("Failed to calculate digest");
                    log::error!("{}", message);
                    APIRegistryError::DigestInvalid {message, details: String::default() }
                })
        },
        DigestAlgorithm::Sha512 => {
            sha512_tag_digest(reader).map_err(|_e| {
                    let message = format!("Failed to calculate digest");
                    log::error!("{}", message);
                    APIRegistryError::DigestInvalid {message, details: String::default() }
            })
